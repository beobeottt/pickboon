...
export const ClubCard = ({ uuid, name, location, members, logo, date, point, price }: Club) => {
  const navigate = useNavigate();

  const handleJoinNow = () => {
    if (!uuid) {
      console.error("UUID is undefined in ClubCard");
      return;
    }
    navigate(`/club/${uuid}`);
  };
  ...
  return (
    <div className="bg-[#23243a] rounded-xl shadow-lg p-6 hover:shadow-xl transition-all duration-300 border border-gray-700 hover:border-red-500">
      ...
      {/* Action Button */}
      <button
        className="w-full mt-6 bg-red-600 hover:bg-red-700 text-white font-semibold py-3 px-4 rounded-lg transition-colors duration-300"
